...
let settings = reactive(JSON.parse(localStorage.getItem("widget_settings_" + id) || "{}") as TotalCounterConfig);
...
function commitSettings(newConfig: TotalCounterConfig): void {
  settings.title = newConfig.title;
  settings.periodSeconds = newConfig.periodSeconds;
  settings.eventTypes = newConfig.eventTypes;
  localStorage.setItem("widget_settings_" + id, JSON.stringify(settings));

  // Need to re-render the widget to update the period seconds
...
<template>
  <span class="static">
    <svg class="fixed z-50 top-0 right-0 m-1 config-button cursor-pointer transition ease-in-out opacity-30 hover:opacity-100"
         @click="() => showConfigureModal('TotalCounterConfig', settings, commitSettings)"
    ...
    <span>
      <TotalAmountOverTimeCounter v-if="configurationIsValid && render" :title="settings.title" :period-seconds=settings.periodSeconds :event-type="settings.eventTypes" />

      <span v-else-if="render" class="border rounded drop-shadow text-center bg-zinc-800 text-red-500 font-bold text-xl h-full flex flex-col justify-center">
